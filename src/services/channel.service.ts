...
export const getChannelInfoService = async (
  ...
  if (!channelData) {
    ...
    const channel = await channelRepository.findActiveChannelById(channelId);
...
export const getFeaturedContentService = async (
  ...
  // find the active channel by name only
  const channel = await channelRepository.findActiveChannelByName(channelId);
  // check if the channel exists or not
  if (!channel) {
    throw new ApiError(404, "Channel not found");
  }
  // get the playlist using aggregation pipeline
  const featuredPlaylist = await channelRepository.getFeaturedPlaylist(
    channelObjectId,
    channel.name
  );
...
export const getVideosService = async (
  ...
  // get the channel id and find the active channel
  const channel = await channelRepository.findActiveChannelById(channelId);
  // check if the channel exists or not
  if (!channel) {
    throw new ApiError(404, "Channel Not Found");
  ...
  const [videos, totalVideos] = await channelRepository.findChannelVideos(
    channelId,
    skip,
    limit
  );
...
export const getPlaylistsService = async (
  ...
  const [playlists, totalPlaylists] =
    await channelRepository.findChannelPlaylist(channelId, skip, limit);
...
export const updateChannelInfoService = async (
  ...
  const channel = await channelRepository.findChannelByOwner(channelId, userId);
  ...
  if (handle && handle !== channel.handle) {
    const handleExists = await channelRepository.handleExists(handle);
  ...
  await channelRepository.updateChannelInfo(channel, { name, handle, bio });
...
export const updateAvatarService = async (
  ...
  await Promise.all([
    channelRepository.updateChannelAvatar(channel, newAvatar),
    channelRepository.syncAvatarWithUser(user, newAvatar),
  ]);

  // after saving the new avatar in Db, invalidate the cached data
  await invalidateCache(`channel-profile:${channelId}`);

  // if uploading successful, delete the old avatar file
  if (oldAvatar) {
...
export const updateCoverImageService = async (
  ...
  await channelRepository.updateCoverImage(channel, {
